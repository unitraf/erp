...
import { deleteClient } from "../../redux/clientUnitraf/action";
const content = [
  {
...
const [alert, setAlert] = useState(false)
  ...
  const optionsContent = [
    { content: "Aperçu", route: "/client" },
    { content: "Editer", route: "/newClient" },
    { content: "Supprimer", route: "/client" },
  ];
  const clientOptions = (item, index) => (
    ...
    >
      <Link to={`${item.route}/${indexClient}`} key={index}>
        ...
        <div className="col-12 ">
          ...
          >
            ...
            <thead>
              ...
              >
                <th className="col-1">NIF</th>
                <th className="col-3">Nom, Raison sociale</th>
                <th className="col-2">Téléphone</th>
                <th className="col-1">BP</th>
                <th className="col-3">Adresse</th>

                <th className="col-1">Exonéré</th>
                <th className="col-1">N° Tiers</th>
                <th className="">...</th>
              </tr>
            </thead>
            {/* body Moodèle gauche*/}
            ...
            >
              {clients.map((item, index) => (
                ...
                >
                  {/* Start exo item */}
                  <td className={`col-1 ${indexClient === index && "select"}`}>
                    {item.nif}
                  </td>
                  <td className={`col-3 ${indexClient === index && "select"}`}>
                    {item.nom}
                  </td>
                  <td className={`col-2 ${indexClient === index && "select"}`}>
                    {item.tel}
                  </td>
                  ...
                  <td className={` ${indexClient === index && "select"}`}>
                    {indexClient === index && (
                      <Menu
                        style={{
                          display: "flex",
    ...
    <div>
      ...
      <div className="card">
        <Onglets
          ...
          menu={ongletOptions}
          active={setStatus}
          ongletHeaders={["Importateurs", "Exportateurs", "+"]}
      ...
      <div className="alertFooter">
        ...
        <button
          ...
          onClick={() => {
            console.log("Oui");
            console.log(clients[indexClient])
            dispatch(deleteClient(clients[indexClient].nif))
           setAlert(false)
          }}
        >
          Oui
